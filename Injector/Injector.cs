...
namespace Launcher
{
    public static class Injector
    {
        ...
        {
            Logger.Log.Write($"Injecting '{fullPathToDLL}' into {targetProcess.ProcessName} (PID: {targetProcess.Id})", Logger.ELogType.Info, rtxtLog);
            ...
            {
                Logger.Log.WriteError($"Could not get handle to target process: {Marshal.GetLastWin32Error()}", rtxtLog);
            ...
            {
                Logger.Log.WriteError($"Could not find LoadLibrary in process: {Marshal.GetLastWin32Error()}", rtxtLog);
                WinAPI.CloseHandle(procHandle);
                return false;
            }
            ...
            {
                Logger.Log.WriteError($"Could not allocate memory in target process: {Marshal.GetLastWin32Error()}", rtxtLog);
                WinAPI.CloseHandle(procHandle);
                return false;
            }
            Logger.Log.Write($"Allocated {((IntPtr)fullPathToDLL.Length + 1)} bytes in target process at address {allocMemAddress.ToString("X2")}", Logger.ELogType.Info, rtxtLog);
            ...
            {
                Logger.Log.WriteError($"Could not write to target process: {Marshal.GetLastWin32Error()}", rtxtLog);
            ...
            {
                Logger.Log.WriteError($"Could not create remote thread in target process: {Marshal.GetLastWin32Error()}", rtxtLog);
                WinAPI.VirtualFreeEx(procHandle, allocMemAddress, (IntPtr)0, (uint)(WinAPI.AllocationType.Release));
                WinAPI.CloseHandle(procHandle);
                return false;
            }
            Logger.Log.Write($"Created remote thread in game process @ LoadLibraryA with pointer to path to DLL string as parameter (thID: {ipThread})", Logger.ELogType.Info, rtxtLog);
            ...
            if (waitValue == WinAPI.WAIT_OBJECT_0)
            {
                ...
                {
                    throw new Exception($"Could not get exit code from remote thread: {Marshal.GetLastWin32Error()}");
                }
                if (exitCode == 0)
                {
