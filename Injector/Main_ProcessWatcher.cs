...
namespace Launcher
{
    public partial class Main
    {
        ...
        public void SetWatchedProcess(string processFileName)
        {
            ...
            Logger.Log.Write("ProcessWatcher set to watch for '" + processFileName + "'", Logger.ELogType.Info);
            if (File.Exists(processFileName))
            {
                ...
                Logger.Log.Write("ProcessWatcher loaded process icon", Logger.ELogType.Info);
            }
            LostActiveProcess();
        }
        ...
        private void SetActiveProcess(Process proc)
        {
            ...
            Logger.Log.Write("ProcessWatcher found game process '" + SelectedProcess.ActiveProcess.ProcessName + "' with PID " + SelectedProcess.PID.ToString(), Logger.ELogType.Info);
        ...
        private void LostActiveProcess()
        {
            ...
            Logger.Log.Write("ProcessWatcher lost game process, beginning search again...", Logger.ELogType.Notification);
            ...
            if (ProcessPollWorker.IsBusy)
            {
                ...
                Logger.Log.Write("ProcessWatcher's worker was busy, spinning until free...", Logger.ELogType.Info);
        ...
        private void ProcessPollWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            Logger.Log.Write("ProcessWatcher's worker beginning scan for '" + processFilePath + "'", Logger.ELogType.Info);
        ...
        private void ProcessPollWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if ((e.Error != null) || (e.Cancelled))
            {
                // TO-DO:
                // handle this
                Logger.Log.Write("ProcessWatcher's worker failed or was cancelled", Logger.ELogType.Notification);
                return;
            }
            if (e.Result != null)
            {
